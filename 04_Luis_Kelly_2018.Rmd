...
```{r z_test}
sim_z <- sim %>%
  # Group by Sample Size and SD
  group_by(samp_size,sd) %>% 
  # Calculate Z 
  mutate(z = (sample_mean-pop_mean)/(pop_sd/sqrt(samp_size))) %>% 
  # Calculate P 
...
We know that increasing our sample size and increasing our alpha value increases the power of our test, but the size of SD doesn't have a noticable effect on the power analysis.
